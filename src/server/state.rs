    ...
    let writer_thread = tokio::task::spawn(async move {
        ...
        while writer_alive.load(Ordering::SeqCst) {
            if let Some(frame) = rx.recv().await {
                // Register the pending request BEFORE the PDU goes on the wire. The
                // peer's response can be read and matched while this task is still
                // between the write and the insert, and the read loop drops any
                // response it finds no pending entry for — leaving the caller to
                // block until its response timer expires.
                if let Some(our_sequence_number) = frame.our_sequence_number {
                ...
                match writer.write(&frame.pdu).await {
                    ...
                    Err(e) => {
                        ...
                        // The PDU never left, so no response can ever arrive. Drop the
                        // registration so the caller fails now instead of waiting out
                        // the full response timer.
                        if let Some(our_sequence_number) = frame.our_sequence_number {
                            writer_pending_requests
                                .lock()
                                .await
