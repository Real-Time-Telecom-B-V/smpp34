    ...
    pub fn decode(pdu: &Vec<u8>) -> Result<CommandHeader, SmppError> {
        if pdu.len() < 16 {
            ...
            let command_header = CommandHeader {
                command_length: u32::from_be_bytes(
                    pdu[0..4]
                        .try_into()
                        .map_err(|_| SmppError::ESME_RINVCMDLEN)?,
                ),
                command_id: u32::from_be_bytes(
                    pdu[4..8]
                        .try_into()
                        .map_err(|_| SmppError::ESME_RINVCMDLEN)?,
                ),
                command_status: u32::from_be_bytes(
                    pdu[8..12]
                        .try_into()
                        .map_err(|_| SmppError::ESME_RINVCMDLEN)?,
                ),
            ...
            if pdu.len() != command_header.command_length as usize {
                error!(
                    "PDU length {} does not match command_length {}",
                    pdu.len(),
...
fn decode_bind_request(
    ...
    let (input, password) =
        parse_c_octet_string_nom(input).map_err(|_| SmppError::ESME_RINVPARLEN)?;
    let (input, system_type) =
        parse_c_octet_string_nom(input).map_err(|_| SmppError::ESME_RINVPARLEN)?;
    let (input, interface_version_bytes) =
        take::<usize, &[u8], nom::error::Error<&[u8]>>(1usize)(input)
            .map_err(|_| SmppError::ESME_RINVPARLEN)?;
    let (input, addr_ton_bytes) = take::<usize, &[u8], nom::error::Error<&[u8]>>(1usize)(input)
        .map_err(|_| SmppError::ESME_RINVPARLEN)?;
    let (input, addr_npi_bytes) = take::<usize, &[u8], nom::error::Error<&[u8]>>(1usize)(input)
        .map_err(|_| SmppError::ESME_RINVPARLEN)?;
    let (_input, address_range) =
        parse_c_octet_string_nom(input).map_err(|_| SmppError::ESME_RINVPARLEN)?;
