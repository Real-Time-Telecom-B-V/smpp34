...
impl unbind {
    ...
    pub(crate) fn decode(header: CommandHeader, pdu: &Vec<u8>) -> Result<unbind, SmppError> {
        if header.command_id == CommandId::unbind as u32 {
            ...
            error!(
                "Passed a non unbind PDU to unbind::decode(), command_id: 0x{:08X}",
                header.command_id
            );
...
impl unbind_resp {
    ...
    pub fn decode(header: CommandHeader, pdu: &Vec<u8>) -> Result<unbind_resp, SmppError> {
        if header.command_id == CommandId::unbind_resp as u32 {
            ...
            error!(
                "Passed a non unbind_resp PDU to unbind_resp::decode(), command_id: 0x{:08X}",
                header.command_id
            );
            Err(SmppError::ESME_RINVCMDID)
        }
...
mod all_unbind_tests {
    #[cfg(test)]
    mod unbind_tests {
        use crate::{unbind, CommandHeader, CommandId, SmppError};
        ...
        fn encode_unbind1() {
            ...
            assert_eq!(
                pdu,
                vec![
                    0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00, 0x12,
                    0x34, 0x45, 0x67
                ]
            );
        }
        ...
        fn decode_unbind() {
            let pdu = vec![
                0x00, 0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00, 0x12, 0x34,
                0x45, 0x67,
            ];
        ...
        fn decode_unbind_with_body() {
            let pdu = vec![
                0x00, 0x00, 0x00, 0x14, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00, 0x12, 0x34,
        ...
        fn unbind_accept() {
            ...
            assert_eq!(
                unbind_resp.header.command_status,
    ...
    mod unbind_resp_tests {
        ...
        fn encode_unbind_resp() {
            ...
            assert_eq!(
                pdu,
                vec![
                    0x00, 0x00, 0x00, 0x10, 0x80, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x04, 0x12,
                    0x34, 0x45, 0x67
                ]
            );
        }
