...
impl bind_transmitter {
    ...
    pub(crate) fn new(sequence_number: u32, system_id: String, password: String, system_type: String, addr_ton: u8, addr_npi: u8, address_range: String) -> bind_transmitter {
        ...
        bind_transmitter {
            header: CommandHeader {
                ...
                command_id: CommandId::bind_transmitter as u32,
...
impl bind_transmitter_resp {
    ...
    pub fn decode(header: CommandHeader, pdu: &Vec<u8>) -> Result<bind_transmitter_resp, SmppError> {
        if header.command_status != SmppError::ESME_ROK as u32 {
            return Ok(bind_transmitter_resp { header, system_id: None, sc_interface_version: None });
        }
        if pdu.len() <= 16 {
        ...
        Ok(bind_transmitter_resp { header, system_id: Some(system_id), sc_interface_version })
    }
