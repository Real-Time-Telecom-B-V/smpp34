...
impl generic_nack {
    ...
    pub fn decode(header: CommandHeader, pdu: &Vec<u8>) -> Result<generic_nack, SmppError> {
        if header.command_id == CommandId::generic_nack as u32 {
            ...
            error!(
                "Passed a non generic_nack PDU to generic_nack::decode(), command_id: 0x{:08X}",
                header.command_id
            );
...
mod generic_nack_tests {
    use crate::{generic_nack, CommandHeader, CommandId, SmppError};
    ...
    fn new_generic_nack() {
        ...
        assert_eq!(
            generic_nack.header.command_id,
            CommandId::generic_nack as u32
        );
        assert_eq!(
            generic_nack.header.command_status,
            SmppError::ESME_RALYBND as u32
        );
    ...
    fn encode_generic_nack() {
        ...
        assert_eq!(
            pdu,
            vec![
                0x00, 0x00, 0x00, 0x10, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05, 0x12, 0x34,
                0x45, 0x67
            ]
        );
    }
    ...
    fn decode_generic_nack() {
        ...
        let decoded = generic_nack::decode(decoded_command_header, &pdu)
            .expect("Unable to decode generic_nack");

        assert_eq!(decoded.header.command_length, 16);
        assert_eq!(decoded.header.command_id, CommandId::generic_nack as u32);
        assert_eq!(
            decoded.header.command_status,
            SmppError::ESME_RALYBND as u32
        );
    ...
    fn decode_generic_nack_with_body() {
        let pdu = vec![
            0x00, 0x00, 0x00, 0x14, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05, 0x12, 0x34,
