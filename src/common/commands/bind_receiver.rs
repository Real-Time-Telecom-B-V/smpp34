...
impl bind_receiver_resp {
    ...
    pub fn decode(header: CommandHeader, pdu: &Vec<u8>) -> Result<bind_receiver_resp, SmppError> {
        if header.command_status != SmppError::ESME_ROK as u32 {
            return Ok(bind_receiver_resp { header, system_id: None, sc_interface_version: None });
        }
        if pdu.len() <= 16 {
        ...
        Ok(bind_receiver_resp { header, system_id: Some(system_id), sc_interface_version })
    }
