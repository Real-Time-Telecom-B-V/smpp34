...
fn parse_alert_notification(header: CommandHeader, pdu: &[u8]) -> IResult<&[u8], alert_notification> {
    ...
    let (pdu, ms_availability_status) = if pdu.len() > 0 {
...
impl alert_notification {
    pub(crate) fn new(sequence_number: u32, source_addr_ton: u8, source_addr_npi: u8, source_addr: String, esme_addr_ton: u8, esme_addr_npi: u8, esme_addr: String, ms_availability_status: Option<u8>) -> alert_notification {
        ...
        alert_notification { 
            header: CommandHeader { 
                command_length: (16 + 2 + source_addr.len() + 1 + 2 + esme_addr.len() + 1 + if ms_availability_status.is_some() {1 } else { 0 }) as u32 ,
...
mod all_alert_notification_tests {
    ...
    mod alert_notification_tests {
        ...
        fn decode_alert_notification() {
            let pdu = vec![0x00, 0x00, 0x00, 0x26, // command_length
            ...
            let decoded = alert_notification::decode(decoded_command_header, &pdu[16..]).expect("Unable to decode alert_notification");
    
            assert_eq!(decoded.header.command_length, 38);
            assert_eq!(decoded.header.command_id, CommandId::alert_notification as u32);
        ...
        fn decode_alert_notification_no_ms_availability_status() {
            let pdu = vec![0x00, 0x00, 0x00, 0x25, // command_length
            0x00, 0x00, 0x01, 0x02, // command_id (alert_notification)
            0x00, 0x00, 0x00, 0x00, // command_status
            0x12, 0x34, 0x56, 0x78, // sequence_number
            0x01, // source_addr_ton
