...
use crate::{CommandHeader, SmppError, common::{decode_bind_request, encode_bind_response, encode_bind_request, parse_c_octet_string_nom}, CommandId};
...
impl bind_transceiver {
    pub(crate) fn new(sequence_number: u32, system_id: String, password: String, system_type: String, addr_ton: u8, addr_npi: u8, address_range: String) -> bind_transceiver {
        ...
        bind_transceiver {
            header: CommandHeader {
                ...
                command_id: CommandId::bind_transceiver as u32,
...
impl bind_transceiver_resp {
    ...
    pub fn decode(header: CommandHeader, pdu: &Vec<u8>) -> Result<bind_transceiver_resp, SmppError> {
        if header.command_status != SmppError::ESME_ROK as u32 {
            return Ok(bind_transceiver_resp { header, system_id: None, sc_interface_version: None });
        }
        if pdu.len() <= 16 {
        ...
        Ok(bind_transceiver_resp { header, system_id: Some(system_id), sc_interface_version })
    }
